...
class Input extends React.Component {
  ...
  fetchCurrentWeather(searchQuery) {
    fetch(
      `https://api.weatherstack.com/current?access_key=ae08e42bc4dccde5cee98742ecdb83a4&query=${searchQuery}`
    )
      .then((res) => res.json())
      .then((res) =>
        this.setState({
    ...
    return (
      <div className="Apps">
        ...
        <div className="search">
          ...
          />
          <button className="search-btn" onClick={this.handleButton}></button>
        </div>

        {!res && button ? <Loading /> : null}

        {res && button ? (
          <Result current={current} location={location} />
        ) : null}

        <p className="searchcopy">
          <AiOutlineCopyright
            color="rgb(73, 40, 73)"
            size={10}
